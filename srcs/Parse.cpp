...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/10/14 16:51:04 by rcherik           #+#    #+#             */
/*   Updated: 2016/10/17 13:47:43 by rcherik          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_instruct     Parse::getInstructions(std::string path, bool info) {
   ...
   if (!ifin.good())
   {
      ...
      ifin.close();
      if (!Parse::_info)
         throw ParseException("\033[31mCan't open " + path +" for reading.\033[0m");
      else
         Parse::_PushMessage("\033[31mCan't open " + path +" for reading.\033[0m");
   }
   if (!ifin.is_open())
   {
      Parse::_count++;
   ...
   if (getline(ifin, line).fail())
   {
      ...
      ifin.close();
      if (!Parse::_info)
         throw ParseException("\033[31mCannot read from file " + path +".\033[0m");
      else
         Parse::_PushMessage("\033[31mCannot read from file " + path +".\033[0m");
   }
...
t_instruct     Parse::getInstructions(bool info) {
   ...
   Parse::_info = info;
   std::string line;
   std::cout << "Control + D to end." << std::endl;
